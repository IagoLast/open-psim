...
func create(seed_value: int, data: Dictionary, _scenario: Dictionary = {}) -> void:
	...
	var terrain: Array = []
	for z: int in range(40):
		for x: int in range(40):
			terrain.append("water" if x >= 30 else ("fertile" if x >= 5 and x <= 17 and z >= 7 and z <= 18 else ("forest" if x >= 3 and x <= 9 and z >= 23 and z <= 34 else "land")))
	state = {"schema":1, "balance_version":definitions.balance.version, "seed":seed_value, "rng_state":str(seed_value), "tick":0, "sequence":0, "next_building":1, "next_citizen":1, "coins":definitions.balance.coins, "inventory":definitions.balance.inventory.duplicate(true), "terrain":terrain, "roads":[], "buildings":[], "citizens":[], "exported":0, "immigration_checks":0, "milestones":[], "alerts":[]}
	_add_building("warehouse", 27, 20)
	_add_building("house", 19, 20)
	_add_building("house", 22, 20)
	_add_building("well", 24, 20)
	for x: int in range(18, 30): state.roads.append(19 * 40 + x)
	for z: int in range(16, 19): state.roads.append(z * 40 + 18)
	rebuild()
	for home: int in [2, 3]:
...
func _add_building(kind: String, x: int, z: int) -> Dictionary:
	var item: Dictionary = {"id":state.next_building, "type":kind, "x":x, "z":z, "active":true, "priority":0, "work":0, "access":-1, "connected":false, "water":false, "present":0, "assigned":0, "block":"", "care_days":0, "produced":0}
...
func edges(item: Dictionary) -> Array:
	...
	for cell: int in cells:
		for next: int in Paths.neighbors(cell):
	...
	for item: Dictionary in state.buildings:
		...
		for cell: int in edges(item):
			if roads.has(cell):
				item.access = cell
				break
	reachable = Paths.distances(building(1).get("access", -1), roads)
	for item: Dictionary in state.buildings:
...
func terrain_error(kind: String, x: int, z: int) -> String:
	...
	if x < 0 or z < 0 or x + size > 40 or z + size > 40: return "Fuera del mapa"
	for cell: int in footprint(kind, x, z):
		if state.terrain[cell] == "water": return "Necesita tierra firme"
		if kind == "farm" and state.terrain[cell] != "fertile": return "Terreno no fértil"
	if kind == "fishery":
	...
	if kind == "lumber":
		...
		for dz: int in range(maxi(0, z - 3), mini(40, z + size + 3)):
			for dx: int in range(maxi(0, x - 3), mini(40, x + size + 3)):
				if state.terrain[dz * 40 + dx] == "forest": forest = true
		if not forest: return "Necesita bosque a 3 casillas"
	return ""
	...
	match operation:
		...
		"road":
			...
			for cell: Variant in command.cells:
				if not cell is int or cell < 0 or cell >= 1600: return result("Fuera del mapa")
				if previous >= 0 and cell != previous and not Paths.neighbors(previous).has(cell): return result("El tramo debe ser continuo")
				previous = cell
		...
		"activity", "priority":
			...
			return result()
		"trade":
			return Economy.validate_trade(self, command)
